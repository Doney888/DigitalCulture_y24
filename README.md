...
Выполните несколько заданий с использованием системы контроля версий Git.
Установите Git и склонируйте репозиторий https://nerc.itmo.ru/teaching/intro/git/repo/exercises.git. Выполните следующие команды в директории с вашим репозиторием:
git config user.name "Ваше имя и фамилия на английском языке"
git config user.email "465265@edu.itmo.ru"
./configure.sh
git start
Условие задания находится в файле README.md в корне репозитория
Для проверки выполнения задания используйте команду git verify. После того, как задание будет выполнено, обновите эту страницу. На ней появится форма для отправки отчёта. Сдайте в качестве отчёта список всех выполненных команд для решения задания.
Для перехода к следующему заданию выполните git start next, для сброса задания — git start exercise-name.
...
#на самом деле я просто открыл vim и там это всё написал...
...
cat equation.txt 
#зашёл в vim и вручную совместил 2 файла
git add equation.txt
git commit -am "add equation.txt"
git verify
```
...
cat README.md
git stash
vim bug.txt #удаляем строчку с багом
git add bug.txt
git commit  -am "add bug.txt"
git stash pop
git add program.txt
vim bug.txt #добавляем в конец строчку, что мы исправили багу
git add bug.txt
git commit -am "last commit before sleep"
git verify
```
...
```
cat README.md
git commit --amend --no-edit --date="Thu Sep 19 23:07:28 1987 +0300"
git verify
```
...
vim file.txt #исправляем ошибку в файле
git add file.txt 
git commit --amend 
git rebase --continue 
vim file.txt #исправляем конфликт
git add file.txt 
git rebase --continue 
git log --oneline #проверяем, что не потеряли коммит
git verify
```
...
```
cat README.md
git reflog #ищем коммит, который нас интересует
git rest --hard 97078ae #восстанавливаем нужный коммит
git verify
```
...
```
cat README.md
git rebase -i HEAD~2 #возле последнего коммита меняем pick на squash, дальше подтверждаем изменения
git verify
```
...
```
cat README.md
git rebase --interactive #переходим на редактирование старого коммита
git add --chmod=+x script.sh #меняем права на исполнение файла при добавлении
git commit --amend
git rebase --skip
git ls-files --stage #проверим, что файл получил права на исполнение
git verify
```
...
```
cat README.md
git add --patch file.txt #с помощью флага s разделяем коммит на блоки, индексируя только те блоки, где есть task1 (на самом деле можно было грепать с помощью ещё одного флага, но строк мало, решил сделать руками)
...
```
cat README.md
git rebase --interactive #в рекдакторе меняем строчки с коммитами местами
git verify
```
...
```
cat README.md
git log --oneline #находим ID первого коммита
git bisect start HEAD 9be1c0b #запускаем бинарный поиск начиная с известного последнего плохого коммита (HEAD) и самого первого коммита (9be1c0b)
git bisect run ./faulty-check #запускаем проверку коммитов с помощью файла faulty-check
#Дальше бинарный поиск сам будет осматривать коммиты, пока не найдёт 1 коммит, где faulty-check выдал ошибку. После этого мы получим вывод следующего содержания:
'''351abc6d2f00f74bc928622f010459f9e5b877f3 is the first bad commit
commit 351abc6d2f00f74bc928622f010459f9e5b877f3
Author: Borisyuk Evgeniy <465265@edu.itmo.ru>
Date:   Sun Sep 22 23:39:01 2024 +0300

    Update useful commands #85

 cool-cheatsheet.txt | 1 +
 1 file changed, 1 insertion(+)
bisect found first bad commit'''
#Бинарный поиск нашёл коммит 351abc6d2f00f74bc928622f010459f9e5b877f3 как первый ошибочный и прекратил работу
git push origin 351abc6d2f00f74bc928622f010459f9e5b877f3:search-improved
```
...
- **Файл с условием**: [ссылка на файл с условием задания](./Lab3/task.pdf)
- **Решение**: [ссылка на файл с решением задания](./Lab3/solution)
- **Решение в pdf**: [ссылка на файл с решением задания](./Lab3/homework.pdf)
...
- **Решение**: Запрос делал в командной строке через curl. Указал метод request PATCH, дополнительно параметр do, заголовки из задания и тип контента и потом JSON параметр
...
- **Решение**: `curl -X POST -L http://10.8.0.1/ -c cookie --max-redirs -1`
...
- **Решение**: С помощью утилиты "dig" и параметра "txt" в консоли прочитал их
...
- **Решение**: Добавил адрес в свой интерфейс в VPN, проверил, что пингуется, отправил и поймал запросы на порту 666. После чего в утилите wireshark начал прослушивать адреса и перехватил отправку с хоста 10.8.0.1 закодированного флага и раскодировал его
...
- **Архив репозитория**: [тык](./intro-2024-ci-465265-master(1).zip)
